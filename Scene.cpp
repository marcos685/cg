...
Scene::Scene(int resolution, Camera *camera, vector<Object *> objects, vector<Light *> lights, double w, double d)
{
...
Color Scene::trace_ray(int x, int y)
{
...
void Scene::set_pixel(GLubyte *pixels, int x, int y, Color color)
{
    y = resolution - 1 - y;
    int position = (x + y * resolution) * 3;
    pixels[position] = floor(color.r * 255);
    pixels[position + 1] = floor(color.g * 255);
    pixels[position + 2] = floor(color.b * 255);
}
